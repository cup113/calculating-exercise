...
function set_rQuesNumber(_rQuesNumber) {
    ...
    if (isNaN(_rQuesNumber))
        Err.error_display("题数必须是整数");
    else if (_rQuesNumber > 1000 || _rQuesNumber < 1)
        Err.error_display("题数必须是1-1000的数");
    else {
...
function set_rItem(_rItem) {
    ...
    if (isNaN(_rItem) || _rItem <= 0 || _rItem >= 11)
        Err.error_display("请选择正确的项目");
    else {
        readyItem = _rItem;
        var $diff = $("#rd-form-diff").html(""), note = dateItemNotes[readyItem - 1];
