 ...
 */
 ...
 function generate_footer(VERSION: string, github_url: string, info_detail: boolean = true, year_end: number = 0, year_start: number = 0): void {
	...
	$footer = $("footer");
	$footer.append($(`<div>&copy; ${(year_start === 0 || year_start === year_end)? "": (year_start.toString() + "-")}${(year_end === 0)? yearnow.toString(): year_end.toString()} ${info_detail?('<a href="https://github.com/cup113" target="_blank">cup11</a>'): "cup11"}</div>`).attr({"child": "1"}));
	if (info_detail) {
	...
	$footer.append($(`<div><span>Version: ${VERSION}</span> <a href="readme.html" target="_blank">使用说明</a> <a href="LICENSE.txt" target="_blank">版权声明</a></div>`).attr({"child": "3"}));
}